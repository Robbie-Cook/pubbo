...
const versionCommand = require("@pubbo/version");
...
class PublishCommand extends Command {
  ...
  initialize() {
    ...
    if (this.options.bump === "from-git") {
      ...
      chain = chain.then(() => versionCommand({ ...this.options, gitTagVersion: false, composed: false }));
    }
    ...
    const mapper = pPipe(
      [
        (pkg) => {
          ...
          // If dryRun enabled, don't publish
          if (pkgOpts.dryRun) {
            this.logger.info("Not publishing to NPM -- dryRun enabled");
            return;
          }
